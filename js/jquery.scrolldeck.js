    ...
    $.scrolldeck = function(options) {
		...
		var init = function() {
			...
			if (lastSlide.outerHeight() < $(window).height()) {
				lastSlide.height(lastSlide.height()+$(window).height()-lastSlide.outerHeight()+scrolldeck.settings.offset);
			}
		...
		function getScrollpoint(n) {
			var scrollTop = $(window).scrollTop()+scrolldeck.settings.offset;
